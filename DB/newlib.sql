...
ALTER TABLE booklist
ADD availability INTEGER DEFAULT 0;

UPDATE `library`.`booklist` SET `availability` = '12' WHERE (`id` = '1');
UPDATE `library`.`booklist` SET `availability` = '11' WHERE (`id` = '3');
UPDATE `library`.`booklist` SET `availability` = '12' WHERE (`id` = '4');
UPDATE `library`.`booklist` SET `availability` = '11' WHERE (`id` = '7');
UPDATE `library`.`booklist` SET `availability` = '10' WHERE (`id` = '14');
UPDATE `library`.`booklist` SET `availability` = '12' WHERE (`id` = '15');
UPDATE `library`.`booklist` SET `availability` = '13' WHERE (`id` = '17');
UPDATE `library`.`booklist` SET `availability` = '14' WHERE (`id` = '18');
UPDATE `library`.`booklist` SET `availability` = '20' WHERE (`id` = '19');
UPDATE `library`.`booklist` SET `availability` = '30' WHERE (`id` = '20');
UPDATE `library`.`booklist` SET `availability` = '21' WHERE (`id` = '47');
UPDATE `library`.`booklist` SET `availability` = '20' WHERE (`id` = '48');
UPDATE `library`.`booklist` SET `availability` = '10' WHERE (`id` = '49');
UPDATE `library`.`booklist` SET `availability` = '20' WHERE (`id` = '50');
-- Triggers 
DELIMITER $$
...
FOR EACH ROW
BEGIN
    -- Insert a notice about the new book in the `st_notices` table
    INSERT INTO st_notices (title, date_of_publish, notice, fine, last_date)
    VALUES (
        CONCAT('New Book: ', NEW.name), CURDATE(),                      
        CONCAT('A new book titled "', NEW.name, '" by ', NEW.Author, ' is now available in the library.'),
        0,DATE_ADD(CURDATE(), INTERVAL 30 DAY)  -- Set last date 30 days from the publish date
    );
END$$
DELIMITER ;
...
-- Events
DELIMITER $$
CREATE EVENT IF NOT EXISTS daily_due_reminder
ON SCHEDULE EVERY 1 DAY
DO
BEGIN
    -- Insert reminders for books due in 3 days or fewer
    INSERT INTO st_notices (title, date_of_publish, notice, fine, last_date)
    SELECT 
        CONCAT('Reminder: Book Due Soon: ', bh.isbn) AS title,  -- Title for notice
        NOW() AS date_of_publish,  -- Publish date for the notice
        CONCAT('Reminder: The book with ISBN ', bh.isbn, ' is due in ', DATEDIFF(DATE_ADD(bh.date_of_taken, INTERVAL 14 DAY), CURDATE()), ' days. Please return it on time.') AS notice,  -- Notice content
        0 AS fine,  -- No fine for reminder
        DATE_ADD(bh.date_of_taken, INTERVAL 14 DAY) AS last_date  -- Due date
    FROM 
        bookhistory bh
    WHERE 
        DATEDIFF(DATE_ADD(bh.date_of_taken, INTERVAL 14 DAY), CURDATE()) <= 3
        AND DATEDIFF(DATE_ADD(bh.date_of_taken, INTERVAL 14 DAY), CURDATE()) > 0;  -- Books with 3 or fewer days left, but not overdue

END $$
DELIMITER ;

DELIMITER $$
CREATE EVENT IF NOT EXISTS fine_calculation
ON SCHEDULE EVERY 1 DAY
DO
BEGIN
    DECLARE base_fine DECIMAL(10,2);
    DECLARE daily_increment DECIMAL(10,2);
    SET base_fine = 20.00;       
    SET daily_increment = 5.00;  

    -- Update the fine in st_notices for overdue books based on bookhistory
    UPDATE st_notices sn
    JOIN bookhistory bh ON sn.id = bh.bid
    SET sn.fine = base_fine + ((DATEDIFF(CURDATE(), DATE_ADD(bh.date_of_taken, INTERVAL 14 DAY)) - 1) * daily_increment)
    WHERE DATEDIFF(CURDATE(), DATE_ADD(bh.date_of_taken, INTERVAL 14 DAY)) > 0;
END $$
DELIMITER ;
